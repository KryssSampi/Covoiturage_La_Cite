...
/**
 * TrendMsg — message de tendance avec couleur et icône.
 * Variantes : up, down, stable, warn.
 */

import React from "react";
import type { TendanceVariant } from "../../types/statistiques.types";
...
const TrendMsg: React.FC<{ variant: TendanceVariant; icon: React.ReactNode; children: React.ReactNode }> = ({
  variant, icon, children,
}) => {
  const s = TREND_STYLES[variant];
  return (
    <div className={`flex items-start gap-2.5 px-3.5 py-2.5 rounded-lg border-l-[3px] text-xs leading-relaxed mx-5 my-3 ${s.bg} ${s.border} ${s.text}`}>
